...
export default function AboutPage() {
  return (
    <main>
      ...
      <section className="section"><div className="container values-grid"><div><p className="eyebrow">How we operate</p><h2>The work is measured in readiness.</h2></div><div className="value-cards">{[["01","Plan the property","Know the site, pressure points, and business priorities before the season."],["02","Stay reachable","Keep operations and clients connected when conditions change."],["03","Document the event","Pair field work with weather, location, condition, and completion records."],["04","Invest in capability","Use modern equipment and specialized tools to improve safety and efficiency."]].map(([n,t,d])=><article key={n}><span>{n}</span><h3>{t}</h3><p>{d}</p></article>)}</div></div></section>
      <section className="cta-band"><div className="container cta-band__inner"><div><p className="eyebrow eyebrow--light">Put experience to work</p><h2>Make your property winter-ready.</h2></div><Link className="button button--light" href="/quote">Request a site review <span>↗</span></Link></div></section>
    </main>
  );
}
