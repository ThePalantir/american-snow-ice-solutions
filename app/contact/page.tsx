...
export default function ContactPage() {
  return (
    <main>
      ...
      <section className="section"><div className="container contact-grid"><div className="contact-primary"><p className="eyebrow major-section-label">Operations center</p><a className="button button--signal contact-link-button" href={company.phoneHref} aria-label={`Call American Snow & Ice Solutions at ${company.phone}`}>Call {company.phone}</a><a className="button button--signal contact-link-button" href={company.emailHref}>Email {company.email}</a></div><div className="contact-card"><span>Office hours</span><strong>Monday–Friday</strong><p>8:00am–5:00pm</p><hr/><span>Winter event support</span><strong>Emergency services</strong><p>Available 24/7</p></div><div className="contact-actions"><Link className="button button--signal" href="/quote">Request a risk consultation <span aria-hidden="true">↗</span></Link><Link className="button button--secondary" href="/partner-network">Service partner network <span aria-hidden="true">→</span></Link></div></div></section>
