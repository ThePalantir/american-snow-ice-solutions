...
export default function WinterRiskPlanPage() {
  return (
    <main>
      ...
      <section className="section section--tint"><div className="container"><div className="section-heading"><p className="eyebrow">Planning inputs</p><h2>What we document before winter.</h2></div><div className="detail-grid">{planningInputs.map(([title, detail], index) => <article key={title}><span>0{index + 1}</span><h3>{title}</h3><p>{detail}</p></article>)}</div></div></section>
